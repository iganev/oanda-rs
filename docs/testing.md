...
|---|---|---|
| Unit tests | `src/**` (`cargo test --lib`) | rate limiter timing (paused tokio time), NDJSON framing edge cases, reconnect state machine (backoff escalation/reset, watchdog, fatal errors), retry policy (error classification, `FatalRetry::Budget`, jitter distribution), primitives/serde helpers |
...
| Stream integration | `tests/streaming.rs` | NDJSON parsing over HTTP, heartbeat surfacing, reconnect + `sinceid` back-fill + deduplication, fail-fast on 401, connect-time retry of a transient 5xx |
