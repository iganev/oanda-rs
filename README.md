...
- **Self-managing streams** — pricing and transaction streams detect stale connections via heartbeats, reconnect with capped exponential backoff (outage-safe: at most one attempt per 5 minutes during e.g. weekend maintenance), and **back-fill missed transactions** via `sinceid` with deduplication. The *initial* connect uses the same policy, so a worker started during an outage waits for the venue instead of exiting. Consuming them is just a `while let` loop.
- **One retry policy** — `Error::is_transient()` classifies failures (5xx, 429, 408 and transport errors are worth retrying; other 4xx are not), and `RetryPolicy`/`retry()` apply the same capped, jittered backoff to ordinary requests — so streams and REST calls obey one rule instead of each caller inventing its own. Long-lived workers can opt into `FatalRetry::Budget` to ride out maintenance windows that answer 4xx.
